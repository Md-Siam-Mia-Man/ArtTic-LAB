...
class MetadataHandler:
    ...
    def create_metadata(self, prompt, negative_prompt, model_name, seed, width, height, steps, cfg_scale, lora_info=None):
        ...
        metadata = {
            ...
            "timestamp_modification": datetime.utcnow().isoformat() + "Z",  # Will be updated when modified
            "arttic_lab_version": "3.0.0",  # Current version
        }
            ...
            if hasattr(image, 'text') and 'parameters' in image.text:
                ...
                if original_hash:
                    ...
                    if original_hash != calculated_hash:
                        print("Warning: Metadata hash mismatch - metadata may have been tampered with")
                        return None
    ...
    def update_modification_timestamp(self, image_path):
        ...
        try:
            ...
            if current_metadata:
                ...
                current_metadata["timestamp_modification"] = datetime.utcnow().isoformat() + "Z"
                
                # Recalculate the hash with the new timestamp
                metadata_copy = current_metadata.copy()
                del metadata_copy['hash']
                metadata_str = json.dumps(metadata_copy, sort_keys=True)
                current_metadata["hash"] = hashlib.sha256(metadata_str.encode()).hexdigest()
